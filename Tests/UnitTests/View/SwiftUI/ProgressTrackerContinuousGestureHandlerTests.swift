...
final class ProgressTrackerContinuousGestureHandlerTests: XCTestCase {
    ...
    func test_drag_along_view() {
        ...
        // When
        sut.onChanged(location: CGPoint(x: 121, y: 0))
        // Then
        XCTAssertEqual(self._currentTouchedPageIndex, 2, "Next select page is 1")
        XCTAssertEqual(self._currentPageIndex, 1, "Current page is not updated")

        // When
        sut.onEnded(location: CGPoint(x: 159, y: 0))
    ...
    private func sut() -> ProgressTrackerContinuousGestureHandler {
        return .init(
